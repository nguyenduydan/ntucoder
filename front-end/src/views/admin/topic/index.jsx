...
export default function Index() {
  ...
  // Query keys
  const listQueryKey = ["topics", currentPage, pageSize, ascending, sortField];
  const searchQueryKey = ["topicsSearch", debouncedKeyword, currentPage, pageSize];
  ...
  // Prefetch next page (list or search)
  useEffect(() => {
    ...
    if (sourceData?.totalPages && currentPage < sourceData.totalPages) {
      ...
      if (isKeywordValid(debouncedKeyword)) {
        queryClient.prefetchQuery({
          queryKey: ["topicsSearch", debouncedKeyword, nextPage, pageSize],
          queryFn: () =>
            Search({
              controller: "Topic",
              keyword: debouncedKeyword,
        ...
        queryClient.prefetchQuery({
          queryKey: ["topics", nextPage, pageSize, ascending, sortField],
          queryFn: () =>
            getList({
              controller: "Topic",
              page: nextPage,
  ...
  const refreshTable = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["topicsSearch"] });
  };
